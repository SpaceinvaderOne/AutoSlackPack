...
RUN slackpkg install binutils gcc-g++ libffi glibc libunistring elfutils guile make gc kernel-headers gcc && \
    ...
    slackpkg install atk at-spi2-atk gtk+3 qt5 alsa-lib libX11 libXau libXdmcp libxcb libXext libXrender libXi libXtst libXcomposite libXcursor libXdamage libXfixes libXinerama libxkbcommon libXrandr libXres libXScrnSaver libXv libXvMC libXxf86vm libdrm mesa vulkan-sdk && \
    slackpkg install flac libogg libvorbis libtheora speex opus x264 x265 ffmpeg boost icu4c json-c yaml-cpp libuv libevent && \
    slackpkg install mariadb-client postgresql-libs unixODBC curl libcurl nmap samba xz lzma gzip bzip2 lzip lzop zstd imagemagick graphicsmagick gnutls nettle p11-kit dbus glib2 libcap nghttp2 pcre pcre2 && \
